...
from actions import Actions, norm
...
class RestoredLogFilter(logging.Filter):
    def filter(self, record):
        return not record.exc_info and record.msg == "RESTORED event=%s"
for handler in logging.getLogger().handlers:
    handler.addFilter(RestoredLogFilter())
...
def assistant_loop(state, overlay, tray, identity):
    try:
        stt = STT()
        tts = TTS()
        actions = Actions(CFG, identity=identity)
        logging.info("RESTORED event=%s", "components_ready")
        brain = Brain(memory_context_provider=actions.memory_context)
        logging.info("Vexi v%s ready channel=%s build=%s pid=%s path=%s", __version__, __channel__, __build__, os.getpid(), BASE)
        overlay.show("Включена", f"{identity.display_name()} v{__version__} {__channel__} готова")
    except Exception as e:
        logging.exception("Startup error: %s", e)
        overlay.show("Ошибка", "Смотри лог")
        tray.set_kind("error")
        return
    ...
    if bool(CFG.get("onboarding_enabled", True)):
        try:
            first_prompt = actions.onboarding_prompt()
        except Exception:
            first_prompt = None
        if first_prompt:
            tray.set_kind("speaking")
            overlay.show("Говорю", first_prompt)
            pending_wav = tts.speak(first_prompt, state)
            tray.set_kind("idle")
            session_until = time.monotonic() + float(CFG.get("question_timeout_seconds", 50))
    ...
    while not state.exit_requested:
        ...
        now = time.monotonic()
        session_active = now < session_until
        timeout = max(0.0, session_until-now) if session_active else None
        ...
        try:
            if pending_wav is not None:
                ...
                wav = record_utterance(state, timeout)
            if wav is None:
                if session_active:
                    session_until = 0.0
                continue
            ...
            if not text:
                continue
            activated, command = extract_activation(text, identity)
            if not activated and not session_active:
                continue
            command = meaningful_speech(command if activated else text)
            command, _ = CANONICALIZER.canonicalize(command)
            text = command

            if activated:
                overlay.show("Слушаю", text)
                tray.set_kind("idle")
            ...
            if activated and not command:
                ...
                pending_wav = tts.speak(answer, state)
                tray.set_kind("idle")
                session_until = time.monotonic() + float(CFG.get("conversation_timeout_seconds", 35))
                continue

            if not activated and session_active:
                command = text

            if is_close_phrase(command):
                answer = "Хорошо, я рядом."
                brain.remember(command, answer)
                overlay.show("Говорю", answer)
                tray.set_kind("speaking")
                pending_wav = tts.speak(answer, state)
                tray.set_kind("idle")
            ...
            chat_key = small_talk_key(command)
            if chat_key:
                handled, answer = True, SMALL_TALK[chat_key]
            else:
                handled, answer = actions.execute(command)

            if not handled:
                if norm(command) in ("ты тут", "ты здесь", "ты на связи", "на связи"):
                    answer = "Да, я тут."
                    brain.remember(command, answer)
                else:
                    tray.set_kind("thinking")
                    overlay.show("Думаю", command)
            ...
            conv = float(CFG.get("conversation_timeout_seconds", 35))
            if "?" in answer:
                conv = float(CFG.get("question_timeout_seconds", 50))
            session_until = time.monotonic() + conv

        except sd.PortAudioError as e:
            logging.warning("Microphone stream interrupted; reopening: %s", e)
            overlay.show("Слушаю", "Переподключаю микрофон")
            time.sleep(0.75)
            tray.set_kind("idle" if state.enabled else "muted")
            continue
        except Exception as e:
